    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div class="flex flex-col justify-center">
          <div class="mb-6">
            <span class="inline-block py-1 px-3 rounded-full bg-indigo-50 text-indigo-600 text-sm font-semibold mb-4">
              {{ product().category }}
            </span>
            <h1 class="text-4xl md:text-5xl font-bold text-slate-900 mb-4">{{ product().name }}</h1>
            <div class="flex items-center gap-4 mb-6">
              ...
              <div class="flex items-center text-amber-400">
                ...
                <span class="ml-1 text-slate-500 font-medium">{{ product().rating }} (120 reviews)</span>
              </div>
            </div>
            <p class="text-slate-600 text-lg leading-relaxed mb-8">
              Experience the depth of emotion and color in this unique piece. 
              Perfect for modern living spaces, this artwork brings a sense of calm and inspiration to any room.
            ...
            <div class="mb-8">
              ...
              <div class="flex gap-3">
                @for (color of product().colors; track color) {
                  ...
                  >
                    <span class="w-8 h-8 rounded-full border border-slate-200" [style.background-color]="color"></span>
                  </button>
                }
              </div>
            </div>
          ...
          <div class="flex flex-col sm:flex-row gap-4 mb-8">
            <div class="flex items-center border border-slate-200 rounded-full px-4 py-2 w-max">
              <button (click)="decrementQty()" class="p-2 text-slate-500 hover:text-indigo-600 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                </svg>
              </button>
              <span class="mx-4 font-bold text-lg w-8 text-center">{{ quantity() }}</span>
              <button (click)="incrementQty()" class="p-2 text-slate-500 hover:text-indigo-600 transition-colors">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <button 
              (click)="addToCart()"
              class="flex-1 bg-slate-900 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-indigo-600 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              class="p-4 border border-slate-200 rounded-full hover:border-pink-500 hover:text-pink-500 transition-colors group"
              [class.bg-pink-50]="isInWishlist"
              [class.border-pink-500]="isInWishlist"
              [class.text-pink-500]="isInWishlist"
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:scale-110 transition-transform" [attr.fill]="isInWishlist ? 'currentColor' : 'none'" viewBox="0 0 24 24" stroke="currentColor">
