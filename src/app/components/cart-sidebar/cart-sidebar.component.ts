  ...
  template: `
    ...
    >
      ...
      <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-white">
        ...
        <button (click)="cartService.toggleCart()" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 rounded-full transition-colors">
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        @if (cartService.items().length === 0) {
          <div class="h-full flex flex-col items-center justify-center text-center space-y-4">
            ...
            <button (click)="cartService.toggleCart()" class="text-indigo-600 font-semibold hover:text-indigo-700">
              Start Shopping
            </button>
          </div>
        } @else {
                  ...
                  <div>
                    ...
                    @if (item.selectedColor) {
                      <div class="flex items-center gap-1 mt-1">
                        ...
                        <span class="w-3 h-3 rounded-full border border-slate-200" [style.background-color]="item.selectedColor"></span>
                      </div>
                    }
                  </div>
                  <button (click)="cartService.removeFromCart(item.product.id, item.selectedColor)" class="text-slate-400 hover:text-red-500 transition-colors">
      ...
      @if (cartService.items().length > 0) {
        <div class="p-6 border-t border-slate-100 bg-slate-50">
          ...
          <button 
            routerLink="/checkout" 
            (click)="cartService.toggleCart()"
            class="w-full py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-indigo-600 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
          >
            Proceed to Checkout
          </button>
        </div>
